...
if ~isfield(obj.bp.autowater,'nums')            % If 'nums' is not a field in obj.bp.autowater...(aka behavior was run on MasterProtocol_v6)
    tmp = obj.bp.autowater;
    obj.bp = rmfield(obj.bp,'autowater');       % Remove the 'autowater' field from the object
    obj.bp.autowater.nums = tmp + (tmp-1)*-2;   % And re-create it such that it contains autowater.nums==1 and 1 (as in old MasterProtocols)
end
