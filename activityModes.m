...
params.condition(3) = {'R&~hit&~stim.enable&autowater.nums==2&~early'};   % error right, no stim, aw off
params.condition(4) = {'L&~hit&~stim.enable&autowater.nums==2&~early'};   % error left, no stim, aw off
...
% colors to use, should be as many entries as in params.conditions
params.colors = {[0 0 1],[1 0 0], ...
                 [0.5 0.5 1],[1 0.5 0.5], ...
...
plt.lw = [2 2];
plt.smooth = 31;
plotAllModes(rez, obj.bp.ev, params.alignEvent, params.colors, plt) 
...
plt.legend = {'Right 2AFC','Left 2AFC','Right Error','Left Error','Right AW', 'Left AW'};
plt.conditions = [1,2,3,4,5,6];
plt.lw = [2.5 2.5 1.5 1.5 1.5 1.5];
plt.smooth = 31;
plotAllModes(rez, obj.bp.ev, params.alignEvent, params.colors, plt) 
