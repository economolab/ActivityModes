function edges = findedges_FirstLick(time,bp,epoch,trial,alignEvent)
    ...
    switch epoch
        ...
        case 'prego'
            e1 = bp.ev.firstLick(trial) - 0.1;
            e2 = bp.ev.firstLick(trial); 
        case 'postgo'
            e1 = bp.ev.firstLick(trial);
            e2 = bp.ev.firstLick(trial) + 0.1; 
        case 'outcome'
            e1 = bp.ev.firstLick(trial)-0.05;
            e2 = bp.ev.firstLick(trial) + 1.3;
        case 'moveonset'
            e1 = bp.ev.moveOnset(trial);
            e2 = bp.ev.moveOnset(trial) + 0.2;
        case 'action'
            e1 = bp.ev.firstLick(trial) - 0.05;
            e2 = bp.ev.firstLick(trial) + 0.15;
    end
